...
public class ListTester {
	...
	private enum Result {
		IndexOutOfBounds, IllegalState, NoSuchElement, ConcurrentModification, UnsupportedOperation, NoException, UnexpectedException, True, False, Pass, Fail, MatchingValue, ValidString
	};
	...
	private static final Integer ELEMENT_X = new Integer(-1);// element that should appear in no
																// lists
	private static final Integer ELEMENT_Z = new Integer(-2);// element that should appear in no
																// lists
	...
	private void runTests() {
		...
		testSingleElementList(A_set0_B, "A_set0_B", LIST_A, STRING_A);
		// 2-element to 1-element
		testSingleElementList(AB_removeB_A, "AB_removeB_A", LIST_A, STRING_A);
		// 2-element to 3-element
		testThreeElementList(AB_addToFrontC_ABC, "AB_addToFrontC_ABC", LIST_A, STRING_A);
		// 2-element to changed 2-element via set()
		testTwoElementList(AB_set0_CB, "AB_set0_CB", LIST_BA, STRING_BA);
		// 3-element to 2-element
		testTwoElementList(ABC_removeA_BC, "ABC_removeA_BC", LIST_BA, STRING_BA);
		// 3-element to changed 3-element via set()
		testThreeElementList(ABC_set0_DBC, "ABC_set0_DBC", LIST_A, STRING_A);
	 ...
	 * list -> constructor -> [ ]
	 * 
	 * NOTE: the return type is a basic IndexedUnsortedList reference, so each test
	 * method will need to cast the reference to the specific interface (Indexed or
	...
	/**
	 * Scenario: [A,B] -> addToFront(C) -> [A,B,C]
	 * 
	 * @return [A,B,C] after addToFront(C)
	 */
	private IndexedUnsortedList<Integer> AB_addToFrontC_ABC() {
		IndexedUnsortedList<Integer> list = emptyList_addToFrontA_A();
		return list;
	}

	private Scenario<Integer> AB_addToFrontC_ABC = () -> AB_addToFrontC_ABC();
	 ...
	 * 
	 * @param scenario
	 *            lambda reference to scenario builder method
	 * @param scenarioName
	 ...
	 *            name of the scenario being tested
	 * @param contents
	 *            elements expected in the list after scenario has been set up
	 * @param contentsString
	 *            contains character labels corresponding to values in contents
	 */
	...
	private void testTwoElementList(Scenario<Integer> scenario, String scenarioName, Integer[] contents, String contentsString) {
		...
		try {
			...
			printTest(scenarioName + "_testAddToFront", testAddToFront(scenario.build(), ELEMENT_B, Result.NoException));
			printTest(scenarioName + "_testAddToRear", testAddToRear(scenario.build(), ELEMENT_B, Result.NoException));
			printTest(scenarioName + "_testAddAfter" + contentsString.charAt(0), testAddAfter(scenario.build(), contents[0], ELEMENT_C, Result.NoException));
			printTest(scenarioName + "_testAddAfter" + contentsString.charAt(1), testAddAfter(scenario.build(), contents[1], ELEMENT_C, Result.NoException));
			printTest(scenarioName + "_testAddAfterX", testAddAfter(scenario.build(), ELEMENT_X, ELEMENT_Z, Result.NoSuchElement));
			printTest(scenarioName + "_testAdd", testAdd(scenario.build(), ELEMENT_C, Result.NoException));
			printTest(scenarioName + "_testAddAtIndexNeg1", testAddAtIndex(scenario.build(), -1, ELEMENT_X, Result.IndexOutOfBounds));
			printTest(scenarioName + "_testAddAtIndex0", testAddAtIndex(scenario.build(), 0, ELEMENT_C, Result.NoException));
			printTest(scenarioName + "_testAddAtIndex1", testAddAtIndex(scenario.build(), 1, ELEMENT_C, Result.NoException));
			printTest(scenarioName + "_testAddAtIndex2", testAddAtIndex(scenario.build(), 2, ELEMENT_C, Result.NoException));
			...
			printTest(scenarioName + "_iterNextNext_testIterNext", testIterNext((iterAfterNext(scenario.build(), 2)), contents[0], Result.NoSuchElement));
			...
			printTest(scenarioName + "_iterNextRemoveNext_testIterHasNext", testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
			printTest(scenarioName + "_iterNextRemoveNext_testIterNext", testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.NoSuchElement));
			printTest(scenarioName + "_iterNextRemoveNext_testIterRemove", testIterRemove(iterAfterRemove(iterAfterNext(scenario.build(), 2)), Result.IllegalState));
		} catch (Exception e) {
			System.out.printf("***UNABLE TO RUN/COMPLETE %s***\n", scenarioName + " TESTS");
			e.printStackTrace();
		} finally {
		...
		try {
			...
			if (str.length() < (list.size() + list.size() / 2 + 2)) { // elements + commas + '[' +
				...
				if (firstChar != '[' || lastChar != ']') {
					result = Result.Fail;
				} else if (str.contains("@") && !str.contains(" ") && Character.isLetter(str.charAt(0)) && (Character.isDigit(lastChar) || (lastChar >= 'a' && lastChar <= 'f'))) {
	 ...
	 * 
	 * @param iterator
	 *            an iterator already positioned for the call to hasPrevious()
	 * @param expectedResult
	 * @return test success
	 */
	 ...
	 * 
	 * @param iterator
	 *            an iterator already positioned for the call to add()
	 * @param element
	 *            new Integer for insertion
	 * @param expectedResult
	 * @return test success
	 */
	 ...
	 * 
	 * @param iterator
	 *            an iterator already positioned for the call to set()
	 * @param element
	 *            replacement Integer for last returned element
	 * @param expectedResult
	 * @return test success
	 */
	 ...
	 * 
	 * @param iterator
	 *            already positioned for the call to nextIndex()
	 * @param expectedIndex
	 * @param expectedResult
	 * @return test success
	 */
	 ...
	 * 
	 * @param iterator
	 *            already positioned for the call to previousIndex()
	 * @param expectedIndex
	 * @param expectedResult
	 * @return test success
	 */
